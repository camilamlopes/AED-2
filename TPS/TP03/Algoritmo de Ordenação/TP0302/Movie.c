...
// Funções de tratamento e escrita dos Filmes
...
int menorIndex(int i, int j){
    if(i == j) return i;

    comp += 1;
    int k = menorIndex(i + 1, j);

    return (strcmp(movies[i].original_title, movies[k].original_title) < 0 ? i : k);
}

void selectionSort(int index, int tam){    
    if(index == tam - 1) return;

    int menorI = menorIndex(index, tam - 1);

    if (menorI != index){
        Movie tmp = movies[menorI];
        movies[menorI] = movies[index];
        movies[index] = tmp;
...
int main() {
    ...
    FILE* log = fopen("689172_selecaoRecursiva.txt", "w");
    ...
    selectionSort(0, count_movies);

    time_t end = time(NULL) - start;
    double tempo = (double)end;

    fprintf(log, "689172\t%d\t%d\t%lf", comp, move, tempo);
