...
class Lista{
    ...
    public Lista (int tam){
        this.array = new Filme[tam];
        this.tam = 0;
    }

    /* MÉTODOS DE INSERÇÃO */
    public void inserirInicio(Filme film) throws Exception{
        if(tam >= array.length){
    ...
    public void inserirFim(Filme film) throws Exception{
        if(tam >= array.length){
            throw new Exception("Erro ao inserir!");
        }
        
        array[tam] = film.clonar();
        tam++;
    }
